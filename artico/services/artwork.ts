...
export const identifyArtwork = async (imageUri: string, language: string = 'en', role: string = 'adult'): Promise<ArtworkInfo> => {
  try {
    ...
    const response = await fetch(`http://${host}:8000/api/recognize`, {
      method: 'POST',
      body: formData,
      headers: {
